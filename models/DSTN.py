...
from models.net import Net, Vgg16
...
class DSTN(BaseModel):
    ...
    def initialize(self, opt):
        ...
        using_new = True
        if using_new:
            self.vggNet = self._load_vgg16_v2()
        else:
    ...
    def _load_vgg16(self):
        model_url = "http://cmp.felk.cvut.cz/cnnimageretrieval/data/networks/imagenet/imagenet-caffe-vgg16-features-d369c8e.pth"
        this_dir = os.path.dirname(__file__)
        pretrain_dir = os.path.join(this_dir, "../pretrain")

        self.vggNet = getattr(torchvision.models, 'vgg16')(pretrained=True)
        self.vggNet = list(self.vggNet.children())[:-2]
        self.vggNet = torch.nn.Sequential(*self.vggNet)
        #self.vggNet.load_state_dict(model_zoo.load_url(model_url, model_dir=pretrain_dir))
        return self.vggNet

    def _load_vgg16_v2(self):
        #
        native_vgg = getattr(torchvision.models, 'vgg16')(pretrained=True)
        native_vgg = list(native_vgg.children())[:-2]
        native_vgg = torch.nn.Sequential(*native_vgg)
        #
        self.vggNet = net.Vgg16()
        #
        native_params = list(native_vgg.parameters())
        new_params = list(self.vggNet.parameters())
        #
        for idx, cur_param in enumerate(new_params):
            cur_param.data.copy_(native_params[idx].data)
        #
        return self.vggNet
    ...
    def backward_G(self):
        ...
        #content realA_fakeB
        real_A_content = Variable(self.vggNet.forward(net.image_content_pre(self.real_A))[2].data, requires_grad=False)
        fake_B_content = Variable(self.vggNet.forward(net.image_content_pre(fake_B))[2].data, requires_grad=True)
        loss_Content_A = self.criterionContent(fake_B_content, real_A_content)
        # content realB_fakeA
        real_B_content = Variable(self.vggNet.forward(net.image_content_pre(self.real_B))[2].data, requires_grad=False)
        fake_A_content = Variable(self.vggNet.forward(net.image_content_pre(fake_A))[2].data, requires_grad=True)
